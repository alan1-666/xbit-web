...
import { EventType, Ohlcdto, Timeframe, TransactionDto } from '@/@generated/gql/graphql-meme2.ts'
import { useSimpleOHLC } from '@hooks/meme/useSimpleOHLC.ts'
...
export const PriceChart = (props: PriceChartProps) => {
  ...
  const filterFn = useCallback(
    (data: Ohlcdto) => {
      const timestamp = +data.ts * 1000
      return timestamp >= firstTs
    },
    [firstTs],
  )

  const { data } = useSimpleOHLC({
    tokenAddress,
    chainId,
    timeframe: timeframeMap[timeRange],
