...
const DoublePrincipalAfterPurchaseExplained = React.memo(() => {
  ...
  return (
    <>
      ...
      >
        <img src="/images/orderSetting/icon-new-info.svg" className="w-[16px] min-w-[16px]" alt="" />
      </div>
      <Drawer open={open} onOpenChange={setOpen} repositionInputs={false}>
        <DrawerContent className="w-full bg-[#232329] max-w-[768px] max-h-[80vh] mx-auto">
          <DrawerHeader>
            <DrawerTitle className="mt-1.5">
              <div className="text-[calc(1rem*(22/16))] leading-[1] app-font-regular text-left">
                {t('orderForm.form.doublePrincipalAfterPurchase')}
              </div>
            </DrawerTitle>
          ...
          <DrawerDescription className="flex flex-col px-4 gap-[12px]">
            <Text text={t('walletDetail.tooltip.doublePrincipal')} fontWeight="light" color="#FFFFFFCC" fontSize={14} />
          </DrawerDescription>
...
DoublePrincipalAfterPurchaseExplained.displayName = 'DoublePrincipalAfterPurchaseExplained'

export default DoublePrincipalAfterPurchaseExplained
