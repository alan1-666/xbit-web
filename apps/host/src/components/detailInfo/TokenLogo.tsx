...
export interface TokenLogoProps {
  ...
  chainLogo: string
  className?: string
  styleAvatarImage?: string
  crossOrigin?: boolean
}

export const TokenLogo = (props: TokenLogoProps) => {
  const { tokenLogo, tokenSymbol, chainLogo, className, styleAvatarImage, crossOrigin } = props
  ...
  return (
    <div className="cursor-pointer relative">
      <Avatar
        className={cn('size-9 block relative', className)}
      ...
      >
        {crossOrigin ? (
          <AvatarImage
            src={tokenLogo}
            alt={tokenSymbol}
            className={cn('size-full border-[0.5px] border-[#2E0066] rounded-[6px] object-cover', styleAvatarImage)}
