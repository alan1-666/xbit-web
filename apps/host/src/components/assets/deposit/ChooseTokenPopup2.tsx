...
import { BLOCKCHAIN_NAMES, BLOCKCHAIN_SHORTNAME, getBlockChainLogo, getBlockchainLogo2 } from '@/utils/helpers.ts'
...
type Props = {
  ...
  onTokenSelected?: (chainId: ChainIds, token: string, walletAddress?: string) => void
  walletBalances: {
...
const nativeTokenSupported = [
  ...
  },
  {
    ...
    chainLogo: getBlockchainLogo2(ChainIds.Arbitrum),
    chainName: 'Arbitrum',
    tokenAddress: USDC_ADDRESS_ARBITRUM, // USDC token address on Arbitrum
...
const ChooseTokenPopup = ({ open, setOpen, onTokenSelected, walletBalances, selectedWallet, type }: Props) => {
  ...
  const chainIdSelectedByWallet = useMemo(() => {
    if (selectedWallet) {
  ...
  useEffect(() => {
    if (selectedWallet) {
      setNetworkSelected(chainIdSelectedByWallet)
    }
  }, [selectedWallet, chainIdSelectedByWallet])
  ...
  useEffect(() => {
    if (open) {
      setSearchValue('')
      setNetworkSelected(chainIdSelectedByWallet || selectedChainId)
      if (inputRef.current) {
  ...
  const handleOnClick = (chainId: ChainIds, token: string, tokenAddress: string, walletAddress?: string) => {
    setOpen(false)
    onTokenSelected?.(chainId, token, walletAddress)
  }
  ...
  return (
    <BottomSheet
      ...
      className="bg-[#232329]"
    >
      <div className={cn(isDesktop ? 'pt-4' : 'h-[75vh]')}>
        <div className={cn('relative w-full mb-[14px]')}>
          <input
            ref={inputRef}
            value={searchValue}
            onChange={(e) => {
            ...
            {listTokens.map((item) => (
              <div key={item.token}>
                <div
                  ...
                  key={item.token}
                  style={{
                    cursor: item.disabled && type === 'withdraw' ? 'not-allowed' : 'pointer',
                  }}
                >
