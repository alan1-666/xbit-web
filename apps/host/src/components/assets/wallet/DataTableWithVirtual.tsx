...
export interface DataTableWithVirtualHandle {
  scrollToTop: () => void
}
...
const DataTableWithVirtual = forwardRef<DataTableWithVirtualHandle, DataTableProps<any, any>>(
  (
    {
      getRowId,
      columns,
    ...
    const renderBody = () => {
      ...
      if (rows.length === 0) {
        return (
          <TableRow className={tableBodyRowClassName}>
            <TableCell colSpan={columns.length} className={cn('h-[calc(100vh-705px)] text-center', tableCellClassName)}>
...
DataTableWithVirtual.displayName = 'DataTableWithVirtual'

export default DataTableWithVirtual
