...
import { addXWalletsFavourite } from '@/hooks/useGetTotalFollowingAddress'
...
async function importFollowingWallets(content: string, chainType: ChainType): Promise<ImportFollowingWalletResp> {
  const followings: FollowingInput[] = content
    ...
    .map((item) => {
      ...
      // const address = idx === -1 ? item : item.slice(0, idx).trim();
      // const alias = idx === -1 ? undefined : (item.slice(idx + 1).trim() || undefined);
      const [address, alias] = item.includes(':') ? item.split(':').map((s) => s.trim()) : [item.trim(), '']
      // const alias = aliasPart && aliasPart !== '' ? aliasPart : undefined;
      return { address: chainType === ChainType.Bsc ? address?.toLowerCase() : address, alias }
    })
  ...
  >({
    ...
    variables: { input: { chain: chainType, followings } },
  })
  addXWalletsFavourite(
    followings.map((item) => ({
...
const ImportExportFollowingWallet = ({ open, setOpen, listFollowing }: ImportExportFollowingWalletProps) => {
  ...
  return (
    <BottomSheet open={open} setOpen={setOpen} title={t('followingWallet.importAndExportWallets')}>
      ...
      <div className="w-full mt-3 pt-3 border-t border-[#ECECED0A] flex gap-4 items-center">
        <Button variant="close" className="rounded-full flex-1 h-11" onClick={onClose}>
          {t('button.cancel')}
        </Button>
