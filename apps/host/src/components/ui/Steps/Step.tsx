...
const Step: React.FC<StepProps> = (props) => {
  ...
  let stepNode: React.ReactNode = (
    <div {...restProps} className={cn('flex flex-col items-center', classString)} style={stepItemStyle}>
      <div onClick={onClick} {...accessibilityProps} className={`${prefixCls}-item-container`}>
        ...
        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-black font-medium ${prefixCls}-item-icon`}>{renderIconNode()}</div>
        <div className={`${prefixCls}-item-content`}>
          <div className={`${prefixCls}-item-title`}>
            {title}
            {subTitle && (
              <div title={typeof subTitle === 'string' ? subTitle : undefined} className={`${prefixCls}-item-subtitle`}>
                {subTitle}
              </div>
            )}
          </div>
