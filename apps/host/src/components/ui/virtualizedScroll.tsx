import { cn } from '@/lib/utils';
import { useVirtualizer } from '@tanstack/react-virtual';
import { HTMLAttributes, Key, useEffect, useRef, useState } from 'react';
import { IconEmpty, IconSpinner } from '../icon';
import { throttle } from 'lodash-es';
import { useStickyScroll } from '@/hooks/useStickyScroll';
import useCustomTranslation from '@/hooks/useCustomTranslation';
import { AnimatePresence } from 'framer-motion';
import { Skeleton } from './skeleton';

interface XVirtualizedScrollProps<T> {
  loading: boolean;
  items: T[];
  renderItem: (item: T, index: number) => React.ReactNode;
  emptyText?: string
  skeletonSize?: number
  animatePresence?: boolean
  useVirtualizedList?: boolean
  loadMoreFn?: () => Promise<boolean> // Function to load more data, returns true if more data is available
  getItemKey?: (index: number) => string // Function to get a unique key for each item
  estimateSize?: number // Function to estimate the size of each item
}
function VirtualizedList<T>(props: XVirtualizedScrollProps<T>) {
  const { items, renderItem, loadMoreFn, getItemKey, estimateSize = 90 } = props
  const [isFetching, setIsFetching] = useState(false)
  const [hasMore, setHasMore] = useState(true)
  const parentRef = useRef<HTMLDivElement>(null)
  const stickyRef = useRef<HTMLDivElement>(null)
  const outerRef = useRef<HTMLDivElement>(null)
  const virtualizer = useVirtualizer({
      count: isFetching ? items.length + 1 : items.length,
      getScrollElement: () => parentRef.current,
      estimateSize: () => estimateSize,
      overscan: 10,
      getItemKey,
    })

  const virtualItems = virtualizer.getVirtualItems()
  const totalSize = virtualizer.getTotalSize() || 1000

  useEffect(() => {
    const stickyElement = document.getElementById('sticky-line') as HTMLDivElement
    const outerElement = document.getElementById('main-content') as HTMLDivElement
    if (stickyElement) {
      stickyRef.current = stickyElement
    }
    if (outerElement) {
      outerRef.current = outerElement
    }
  }, [])

  useEffect(() => {
    const lastItem = virtualItems[virtualItems.length - 1]
    if (!lastItem) return
    const ratio = lastItem.index / virtualItems.length
    const throttleHandler = throttle(() => {
      setIsFetching(true)
      loadMoreFn?.()
        .then(setHasMore)
        .finally(() => setIsFetching(false))
    }, 200)
    if (ratio >= 0.75 && !isFetching && hasMore && loadMoreFn) {
      throttleHandler()
    }
  }, [virtualItems, isFetching, hasMore, loadMoreFn])
  useStickyScroll({
    innerRef: parentRef,
    outerRef: outerRef,
    stickyRef: stickyRef,
  })
  return (
    <div ref={parentRef} className="h-full overflow-y-auto no-scrollbar pb-4 overscroll-y-auto">
      <div style={{ height: totalSize }} className="w-full relative">
        <LoadMore />
        {virtualItems.map((item) => {
          const currentItem = items[item.index]
          return (
            <div
              key={item.key}
              className="w-full pb-[5px] absolute top-0 left-0"
              style={{
                height: `${item.size}px`,
                transform: `translateY(${item.start}px)`,
              }}
            >
              {currentItem ? renderItem(currentItem, item.index) : <LoadMore />}
            </div>
          )
        })}
      </div>
    </div>
  );
}

const LoadMore = (props: HTMLAttributes<HTMLDivElement>) => {
  return (
    <div className="h-full w-full flex justify-center items-center" {...props}>
      <IconSpinner className="size-4 animate-spin" />
    </div>
  )
}


const ListCoinSkeleton = (props: { skeletonSize?: number }) => {
  const { skeletonSize = 10 } = props
  return (
    <div className="space-y-[5px]">
      {Array.from({ length: skeletonSize }).map((_, index) => (
        <Skeleton key={index} className="h-[87px]" />
      ))}
    </div>
  )
}

const EmptyList = (props: { emptyText?: string }) => {
  const { emptyText } = props
  const { t } = useCustomTranslation()
  return (
    <div className="flex flex-col items-center justify-center h-80">
      <IconEmpty />
      <span className="text-[#FFFFFF80] text-[0.75rem]">{emptyText ?? t('listCoin.noData')}</span>
    </div>
  )
}

const AnimatePresenceList = <T,>(props: XVirtualizedScrollProps<T>) => {
  const { items, renderItem } = props
  return <AnimatePresence>{items.map((item, index) => renderItem(item, index))}</AnimatePresence>
}

function XVirtualizedScroll<T>(props: XVirtualizedScrollProps<T>) {
  const { loading, items, renderItem, emptyText, skeletonSize, animatePresence, useVirtualizedList } = props
  if (loading) {
    return <ListCoinSkeleton skeletonSize={skeletonSize} />
  }

  if (!items.length) {
    return <EmptyList emptyText={emptyText} />
  }

  if (useVirtualizedList) {
    return <VirtualizedList {...props} />
  }

  if (animatePresence) {
    return <AnimatePresenceList {...props} />
  }

  return (
    <div className="flex flex-col gap-[5px]">
      {items.map((item, index) => (
        <div key={index} className="mb-[5px]">
          {renderItem(item, index)}
        </div>
      ))}
    </div>
  )
}


export default XVirtualizedScroll;