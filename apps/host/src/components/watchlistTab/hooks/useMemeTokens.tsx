...
export const useMemeTokens = (options: UseMemeTokensOptions = {}) => {
  ...
  // 强制使用 SOLANA 链，不依赖 activeChain
  const forceChain = TYPE_CHAIN.SOLANA

  const dexList = useMemo(() => {
    // 对于 SOLANA 链，使用 solDexList
    return filter.solDexList
  }, [filter])

  const queryInput = useMemo(() => {
    return {
      chain: listCoinHelper.getChainType(forceChain), // 强制使用 SOLANA
  ...
  }, [forceChain, currentTimeframe, filter, direction, dexList])

  console.log('Meme Tokens filter', filter)
  console.log('Meme Tokens Query Input:', queryInput)
