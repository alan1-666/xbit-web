  ...
  const columns: ColumnDef<OrderTradeItem>[] = [
    ...
    },
    {
      ...
      header: () => (
        <div className="flex items-center justify-center">
          <Text text={t('futuresDetails.common.direction')} fontSize={11} fontWeight="medium" color="#FFFFFF80" />
        </div>
      ),
      cell: ({ getValue }) => {
        return (
          <div className="flex items-center justify-center">
            <Text
              text={getValue() === 'Buy' ? t('futuresDetails.common.buy') : t('futuresDetails.common.sell')}
              fontSize={13}
              fontWeight="regular"
              color={getValue() === 'Buy' ? 'var(--desktop-rise)' : 'var(--desktop-fall)'}
            />
          </div>
        )
      },
    },
    {
      ...
      header: () => (
        <div className="flex items-center justify-start pl-[20%]">
          <Text text={`${t('futuresDetails.common.price')} (${baseCoin})`} fontSize={11} fontWeight="medium" color="#FFFFFF80" />
        </div>
      ),
      cell: ({ row, getValue }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ getValue }) => {
        return (
          <div className="flex items-center justify-end">
            <Text text={getValue() as string} fontSize={13} fontWeight="regular" />
          </div>
        )
      },
    },
    ...
    <>
      {isActive && (
        <TableVirtual
          ...
          containerClassName="border-none _hidescrollbar -mt-3"
          tableClassName=""
          tableHeaderRowClassName="text-[calc(1rem*(11/16))] text-[#FFFFFF80] whitespace-nowrap !border-b-0"
          tableHeadClassName="app-font-light !px-[10px] py-1 items-center justify-center pb-0"
          tableBodyClassName=""
          tableBodyRowClassName="group whitespace-nowrap !border-[#ECECED14]"
          tableCellClassName="group-hover:!bg-[#27272a] cursor-pointer px-[10px] py-[8px]"
