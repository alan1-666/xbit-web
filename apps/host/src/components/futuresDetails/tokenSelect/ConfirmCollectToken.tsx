...
import { useToast } from '../tokenSearchDrawer/CustomToast'
...
const ConfirmCollectToken = (props: {
  ...
  // Accept partial symbol info to be compatible with slimmer data passed from ContractList
  tokenInfor: Partial<ISymbolList> & { symbol: string }
}) => {
  const {
    token,
    defaultCollect,
  ...
  const addToFavorites = () => {
    ...
    handleCollectToken({ variables: { input: { symbol: [token], isFavorite: true } } })
      .then(() => {
        // 新收藏的放在最前面
        handleUpdateCache([tokenInfor as ISymbolList, ...favorites])
        showToast({
          type: 'success',
  ...
  useEffect(() => {
    ...
    activeImg.src = '/images/icons/vector-star-icon-active.svg'
    ...
    <>
      <div className={cn('p-2 pr-0 cursor-pointer w-[20px]', triggerClassName)}>
        ...
        <img
          ...
          src={isCollect ? '/images/icons/vector-star-icon-active.svg' : '/images/icons/star-InSearch-icon.svg'}
          alt=""
        />
        {/* )} */}
      </div>
