...
import { formatPrice, handleSizeChange, isHyperOrderSuccess } from '@/components/futuresDetails/trade/tools'
...
import { useOpenOrders } from '@components/futuresDetails/trade/OpenOrdersContext'
import { Dispatch, SetStateAction, useEffect, useMemo, useRef, useState } from 'react'
import { Controller, FormProvider, useForm } from 'react-hook-form'
import { useTranslation } from 'react-i18next'
import { toast } from 'sonner'
import { RawItem } from '../trade/MyPositionList/TpslButton'
...
import { buildTpslOrders } from '../trade/MyPositionList/BuildTpslOrders'
...
import { useTpslMutation } from '../hooks/useAsyncMutation'
...
const TpslDialog = ({ info, open, baseCoin, setOpen, setInfo }: TpslButtonPrpos) => {
  ...
  const tpPricePercentValue = watch('tpPricePercent')
  ...
  const processNextCancel = () => {
    ...
    const needCancelOrder = openOrders.filter((item: any) => item.oid === orderId)
      ...
      >
        <DialogHeader className="border-b border-[#ECECED0A] pb-0">
          <DialogTitle className="py-3 px-3.5 flex w-full items-center justify-between">
            <p className="text-[calc(18rem/16)] py-3">{t('position.setupTPSL')}</p>
            <img
