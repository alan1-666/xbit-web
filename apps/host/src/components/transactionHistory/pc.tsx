...
const TransactionHistory = () => {
  ...
  const maxHeight = useMaxHeightInGridLayout({ containerRef, padding: 80 })
  ...
  const fetchTransactionHistory = async () => {
    try {
      const queryInput: SearchOrderInput = {
        chain: getChainType(activeChain),
      ...
      if (response?.data?.getAllTransactions?.length < LIMIT_PER_PAGE) {
  ...
  useEffect(() => {
    ...
    const mainScrollContainer = document.getElementById('meme-bottom-tabs')
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      const newOrder = event?.data?.data as PendingOrder
    ...
    <div style={{ maxHeight: maxHeight }}>
      ...
      <AnimatePresence>
        {tokenFilter === address && (
          ...
          >
            <div className="mt-[10px] px-3 flex items-center justify-between gap-3">
              <div
                className={cn(
              ...
              >
                <div className="text-[12px] font-[400]">{t('walletDetail.holderTable.totalBuy')}</div>
                <div className="text-rise mt-2 text-[12px] font-[600]">
                  {dataUnit === 'USD'
              ...
              >
                <div className="text-[12px] font-[400]">{t('walletDetail.holderTable.totalSell')}</div>
                <div className="text-fall mt-2 text-[12px] font-[600]">
                  {dataUnit === 'USD'
      ...
      <div id="transaction-history-wrapper" className="border-0 pt-2">
        <TransactionHistoryTable
          ...
          loaderRef={loaderRef}
          isCurrentToken={tokenFilter === address}
        />
      </div>
    </div>
  )
