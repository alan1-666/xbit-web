...
export type TrustWalletAdapterConfig = object

export const TrustWalletName = 'Trust' as WalletName<'Trust'>

function openTrustWalletDeeplink() {
  const dappUrl = encodeURIComponent(window.location.href)
  const deeplink = `https://link.trustwallet.com/open_url?coin_id=501&url=${dappUrl}`
...
export class TrustWalletAdapter extends BaseMessageSignerWalletAdapter {
  ...
  async connect(): Promise<void> {
    try {
      ...
      const wallet = window.trustwallet?.solana || window.solana
      ...
      if (this._isMobileDevice()) {
        if (wallet) {
          ...
          this._publicKey = new PublicKey(wallet.publicKey.toBytes())
          this._wallet = wallet
      ...
      // Desktop
      if (!wallet || wallet?.isOKExWallet || wallet?.isOKExWallet || wallet?.isPhantom) {
