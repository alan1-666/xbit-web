...
export const TokenPocketWalletIcon =
...
export class TokenPocketWalletAdapter extends BaseMessageSignerWalletAdapter {
  ...
  async connectWalletByDeepLink() {
    return new Promise((resolve) => {
      ...
      const deepLink = 'tpoutside://pull.activity?param=' + encodeURIComponent(JSON.stringify(param))
  ...
  async connect(): Promise<void> {
    try {
      ...
      this.connectWalletByDeepLink()
      return
      if (this.readyState === WalletReadyState.Loadable && _isMobileDevice()) {
        const isAppInstalled = await this.connectWalletByDeepLink()
        if (!isAppInstalled) {
          toast.error('您未安装' + TokenPocketWalletName)
          // return
