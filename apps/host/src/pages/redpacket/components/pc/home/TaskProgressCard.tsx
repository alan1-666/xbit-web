...
const TaskItem = ({ type, title, sub_title, reward, done }: { type: string; title: string; sub_title: string, reward: string; done?: boolean }) => {
  ...
  return (
    <div className={`self-stretch p-3 ${done ? 'bg-green-950' : 'bg-neutral-800'} rounded-[20px] shadow-[inset_0px_-7px_15px_0px_rgba(255,255,255,0.06)] outline-1 outline-offset-[-1px] ${done ? 'outline-white/10' : ''} inline-flex justify-between items-center overflow-hidden`}>
      <div className="flex justify-start items-center gap-2">

        <img className="w-10 h-10"  src="/images/redpacket/galxe.svg" alt="Galxe Icon"/>

        <div className="inline-flex justify-start items-center gap-1">
          <div className="text-white text-sm font-normal leading-4">{title}</div>
          {/* <div className="self-stretch opacity-60 justify-start text-white text-xs font-normal leading-4">{reward}</div> */}
          <div className='inline-flex justify-start items-center text-sm font-semibold gap-1'>
            {'->'} 
...
const TaskProgressCard = ({ tasks }: { tasks: any[] }) => {
  const { t } = useTranslation()
  return (
    <div className="w-full self-stretch px-3 py-6 bg-zinc-950 rounded-[20px] shadow-[inset_0px_-7px_15px_0px_rgba(255,255,255,0.06)] outline-1 outline-offset-[-1px] outline-white/20 inline-flex flex-col justify-start items-start gap-5 overflow-hidden">
