...
import DownloadAppModal from '../home/DownloadAppModal'
...
const HistoryItem = ({ iconType, title, label, time, amount, status }: { iconType: ActivityType; title: ActivityType; label?: { text: string; color: string }; time: string; amount: string; status: ActivityStatus}) => {
    ...
    if (status === ActivityStatus.Pending) {
      return (
        <div className="inline-flex justify-end items-center gap-1">
          ...
          <div className="text-right justify-start text-amber-500 text-[14px] font-medium  leading-4">{t('red.packet.pending')}</div>
        </div>
      );
    } else {
      ...
      return (
        <div className="inline-flex justify-end items-center gap-1">
          <div className={`text-right justify-start ${textColor} text-[14px] font-medium leading-4`}>{amount}</div>
        </div>
      );
    }
  };
  ...
  return (
    <div className="self-stretch px-4 py-3 border-b border-white/10 inline-flex justify-between items-center">
      <div className="flex justify-start items-center gap-2">
        ...
        <div className="inline-flex flex-col justify-start items-start gap-1">
          <div className="inline-flex justify-start items-center gap-2">
            <div className="justify-start text-white text-[14px] font-medium  leading-4">{title}</div>
            {label && (
              <div className={`px-1 py-0.5 bg-${label.color}/20 rounded flex justify-center items-center gap-0.5`}>
                ...
                <div className={`justify-start text-${label.color} text-[12px] font-normal  leading-3`}>{label.text}</div>
              </div>
            )}
          </div>
          <div className="justify-start text-neutral-500 text-[12px] font-normal  leading-3">{time}</div>
        </div>
      </div>
      <div className="inline-flex flex-col justify-start items-end gap-1">
        {renderStatusIcon()}
        <div className="text-right justify-start text-neutral-500 text-[12px] font-normal  leading-4">{renderStatus()}</div>
      </div>
    </div>
  );
};
...
const RewardsPage = ({ withdrawalThreshold, withdrawalEnabledWeb }: RewardsPageProps) => {
  ...
  return (
    <div className="flex flex-col h-full px-[40px] pb-[20px]">
      <div className="w-full  py-8 inline-flex flex-col justify-start items-end gap-6 overflow-hidden">
        <div className="self-stretch flex flex-col justify-start items-start gap-1.5">
          <div className="justify-start text-white text-[40px] leading-[40px] font-medium mb-[8px]">{t('red.packet.my.rewards')}</div>
          <div className="opacity-60 justify-start text-white text-[16px] leading-[16px] font-normal ">{t('red.packet.view.balance.history.tips')}</div>
        </div>
        <div className="w-full gap-6 ">
          <div className="w-full aspect-[305/142] relative bg-[url('/images/redpacket/pc-bg-2.png')] bg-cover bg-center px-5 h-[270px]  bg-gradient-to-b from-zinc-950 to-zinc-950/0 rounded-[20px] shadow-[inset_0px_11px_30px_0px_rgba(255,42,106,0.16)] outline-1 outline-offset-[-1px] outline-white/20  py-8  rounded-3xl  flex flex-col justify-center items-center gap-12 overflow-hidden">
            <div className="self-stretch flex flex-col justify-start items-start gap-8">
              <div className="self-stretch flex items-center justify-between gap-2">
                <div className="inline-flex justify-start items-center gap-2">
                  ...
                  <div className="opacity-60 justify-start text-white text-[14px] font-normal ">{t('red.packet.packet.balance')}</div>
                </div>
                <div className="opacity-60 justify-start text-white text-[14px] font-normal ">{t('red.packet.total.withdrawn')}: {redpacketStatus?.totalWithdrawn || 0}</div>
              </div>
              <div className="self-stretch flex flex-col justify-start items-start gap-2">
                <div className="inline-flex justify-start items-end gap-[3px] h-[36px]"> 
                  ...
                  : 
                  <>
                    <div className="text-right justify-start text-white  text-[36px] font-bold leading-none">{ availableBalance}</div>
                    <div className="flex justify-center items-center gap-2.5">
                      <div className="justify-start text-white text-[12px] font-normal leading-none">USDC</div>
                    </div>
                  </>
                   }
                </div>
                <div className="self-stretch opacity-60 justify-start text-white text-[14px] font-normal ">{t('red.packet.withdraw.threshold')}: {withdrawalThreshold} USDC</div>
              </div>
            </div>
                    ...
                    availableBalance < withdrawalThreshold && (() => {
                      ...
                      return (
                        <div className="justify-start">
                          <span className="text-yellow-400 text-[12px] font-medium"> {`${t('red.packet.need.title')}: ${formattedDiff}`}</span>
                          <span className="text-white text-[12px] font-medium"> {t('red.packet.need.more.to.withdraw')}</span>
                        </div>
                      );
                    })()
                  }
                  {
                    availableBalance >= withdrawalThreshold && <div className="justify-start inline-flex items-center" onClick={() => hanleWithdrawClick()}>
                      <img className="w-6 h-5 mr-2.5" src="/images/redpacket/gift.png"/>
                      <span className="text-emerald-500 text-[12px] font-medium cursor-pointer text-white">{t('red.packet.claim')}</span>
                    </div>
                  }
                </div>
              </div>
          ...
          <div className="flex-1  self-stretch rounded-[20px] flex flex-col justify-start items-end gap-6 mt-[24px]">
            <div className="self-stretch bg-neutral-900 rounded-2xl flex flex-col justify-start items-center h-[669px] overflow-y-auto">
              <div className="self-stretch px-4 pt-6 pb-3 rounded-tl-3xl rounded-tr-3xl border-b border-white/10 flex flex-col justify-start items-start gap-1">
                <div className="self-stretch justify-start text-white text-[20px] font-semibold ">{t('red.packet.history')}</div>
                <div className="self-stretch justify-start text-neutral-500 text-[12px] font-medium ">{t('red.packet.packets',{num: summary.totalPackets})}, {t('red.packet.withdrawals',{num: summary.totalWithdrawalSuccess})}</div>
              </div>
              {
                loading ? (
            ...
            <div className="self-stretch px-5 py-4 bg-yellow-300/10 rounded-2xl outline outline-1 outline-offset-[-1px] outline-yellow-300/30 flex flex-col justify-start items-start gap-2.5 overflow-hidden">
              <div className="self-stretch inline-flex justify-start items-center gap-2">
                ...
                <div className="flex-1 justify-start text-yellow-300 text-[14px] font-normal ">{t("red.packet.v2.coming.soon")}</div>
              </div>
            </div>
          </div>
        </div>
