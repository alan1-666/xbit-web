...
const useLoserData = (symbolData: ISymbolList[]) => {
  const { isLoading, symbolList } = useHandleGetData({ condition: 'loser' })
  const currentData = useMergedData(symbolList, symbolData)
...
const LoserList = ({
  ...
  const currentData = useMergedData(loser, symbolData)
  ...
  useEffect(() => {
    const initializeSymbolList = async () => {
      ...
      const cacheLoaded = await loadSymbolListFromCache('loser', filter)

      if (!cacheLoaded?.length) {
        await handleGetSymbolList(filter)
      } else {
        dispatch(setData({ condition: 'loser', data: cacheLoaded as ISymbolList[] }))
        setIsLoadingSymbol(false)
      }
    }
  ...
  return (
    <div className="relative pt-2">
      ...
      <TableVirtual<ISymbolList, any>
        ...
        tableHeaderClassName="text-[#5E5C66] text-[calc(1rem*(12/16))] font-[400] !top-[36px]"
        rowHeight={61}
      />
    </div>
  )
