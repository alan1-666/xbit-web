import HeaderWithBack from '@components/header/HeaderWithBack.tsx'
import { NotificationList } from '@components/notifications/NotificationList.tsx'
import { useTranslation } from 'react-i18next'
import { Link, useLocation } from 'react-router-dom'
import { APP_PATH } from '@/lib/constant.ts'

const Right = ({ search }: { search: string }) => {
  return (
    <Link to={`${APP_PATH.MEME_NOTIFICATION_SETTINGS}${search}`} className="text-white">
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path
          d="M3 9.11035V14.8804C3 17.0004 3 17.0004 5 18.3504L10.5 21.5304C11.33 22.0104 12.68 22.0104 13.5 21.5304L19 18.3504C21 17.0004 21 17.0004 21 14.8904V9.11035C21 7.00035 21 7.00035 19 5.65035L13.5 2.47035C12.68 1.99035 11.33 1.99035 10.5 2.47035L5 5.65035C3 7.00035 3 7.00035 3 9.11035Z"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15Z"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    </Link>
  )
}

export const NotificationsPage = () => {
  const { t } = useTranslation()
  const location = useLocation()
  const searchParams = new URLSearchParams(location.search)
  const backHref = searchParams.get('backHref')

  return (
    <div className="h-dvh overflow-hidden no-scrollbar flex flex-col pb-6">
      <HeaderWithBack
        title={t('notifications.title')}
        className="bg-transparent"
        right={<Right search={location.search} />}
        backHref={backHref ? `${backHref}${location.search}` : APP_PATH.MEME_DISCOVER}
        rightClassName="justify-end"
      />
      <NotificationList />
    </div>
  )
}
