...
import { ContactUsSection } from '@components/settings/ContactUsSection.tsx'
import { ClearCacheDialog } from '@components/settings/ClearCacheDialog.tsx'
...
import { MemeDiscoverDesktopPage } from '../meme/discover/desktop'
...
export const AboutUsPage = () => {
  ...
  const items = [
    // {
    //   key: 'rateXbit',
    //   title: t('appSettings.aboutUs.rateXBit'),
    // },
    {
      key: 'privacy',
      title: t('appSettings.aboutUs.privacyPolicy'),
      onClick: () => {
        navigate(APP_PATH.PRIVACY_POLICY)
      },
    },
    {
      key: 'terms',
    ...
    // {
    //   key: 'clearCache',
    //   title: t('appSettings.aboutUs.clearCache'),
    //   sub: '0.00MB',
  ...
  if (isDesktop) {
    // return <MemeDiscoverDesktopPage />
    navigate(APP_PATH.MEME_DISCOVER)
  }

  return (
    <div className="w-full h-dvh">
      <HeaderWithBack title={t('appSettings.aboutUs.title')} className="bg-transparent" />
      <div className="px-3 py-6 space-y-4">
        <div className="flex flex-col items-center gap-1.5">
          ...
          {/* {Configs.commitSHA && (
            <div className="text-[#FFFFFFB2] text-[calc(14rem/16)] flex items-center">
              ({Configs.commitSHA.slice(0, 6) + '...' + Configs.commitSHA.slice(-6)}{' '}
              <CopyButton
                text={Configs.commitSHA}
                icon="/images/icons/ic-copy2.svg"
                containerClassName="w-4 h-4 ml-[2px]"
                className="block"
              />
              )
            </div>
